...
2. 指定されていなければ `get_activities()` で候補を表示し、ユーザーに選んでもらう。`is_heartbeat_active` が `true` のものは「作業中（別セッション）」セクションに分離して表示する:
   ```
   ### 作業中（別セッション）
   | # | ID  | タイトル            | status      |
   |---|-----|---------------------|-------------|
   | 1 | 475 | [作業] ハートビー... | in_progress |
   ...
   ```
   `is_heartbeat_active` が `true` のアクティビティが1件もなければ「作業中（別セッション）」セクションは省略する。
