...
ユーザーのメッセージからキーワードを抽出し、アクティブコンテキストに該当トピック/アクティビティがあれば
...
**出力手順:**
1. この応答がどのトピックに属するか判断します
2. 応答の**最後**にメタタグを出力します
...
既存トピックが合わない場合は`add_topic`で新規作成します。
メタタグにはトピック名を書くだけで十分ですが、記録（`add_decision`/`add_log`）には
topic_idが必要なので、記録が必要になった時点で`add_topic`を呼んでください。

## 記録

`add_topic`・`add_decision`・`add_log`の3つが記録の基本ツールです。
- `add_topic`: 新しいトピックを作成する。decision/logにはtopic_idが必要なので、記録の前にまず作成する
- `add_decision`: 合意事項を記録する（結論）
- `add_log`: 議論の経緯や生情報を記録する（過程）
...
アクティビティとして作成してください。
`[議論]`アクティビティは軽量なので、迷ったら作って構いません。

アクティビティはデフォルトでは3つのフェーズを持ちます: **議論 → 設計 → 作業**。
...
- **設計**: どう実装するかをユーザーと合意し、作業アクティビティを作ります。
  決して急がず、ユーザーが納得するまで辛抱強くサポートしてください。
  作業アクティビティには詳しい背景情報を書いてください。ほとんどの場合、実装は別のAIが担当します。
- **作業**: 着手前にアクティビティの仕様と関連するdecisionを確認します。
